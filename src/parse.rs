use crate::{error, token::{Span, Token, Tokenizer}};
use anyhow::Result;
...
#[derive(Debug)]
pub enum Expr<'i> {
    Instantiate(Name<'i>, Args<'i>),
    Id(Name<'i>),
}

impl<'i> Expr<'i> {
    pub fn parse(tokens: &mut Tokenizer<'i>) -> Result<Expr<'i>> {
        let id = Name::parse(tokens)?;
        if tokens.eat(Token::LeftBrace)? {
            Args::parse(tokens).map(|args| Expr::Instantiate(id, args))
        } else {
            Ok(Expr::Id(id))
        }
    }
    ...
    pub(crate) fn span(&self) -> Span {
        match self {
            Expr::Instantiate(name, args) => name.span,
            Expr::Id(name) => name.span,
        }
    }
}
...
pub enum ArgKind<'i> {
    Unnamed(Expr<'i>),
    Named { name: Name<'i>, with: Expr<'i> },
}

impl<'i> Arg<'i> {
    fn parse_named(tokens: &mut Tokenizer<'i>, docs: Docs<'i>, first: Expr<'i>) -> Result<Arg<'i>> {
        tokens.expect(Token::Colon)?;
        let expr = Expr::parse(tokens)?;
        match first {
            Expr::Instantiate(id, _args) => {
                Err(error::syntax(tokens, "", Some((id.span, "expression"))).into())
            }
            Expr::Id(name) => Ok(Arg {
                docs,
                kind: ArgKind::Named { name, with: expr },
            }),
        }
    }
    ...
    fn parse(tokens: &mut Tokenizer<'i>, docs: Docs<'i>) -> Result<Arg<'i>> {
        ...
        match tokens.clone().next()? {
            Some((_span, Token::Comma | Token::RightBrace)) => Ok(Arg {
                docs,
                kind: ArgKind::Unnamed(arg),
            }),
            Some((_span, Token::Colon)) => Arg::parse_named(tokens, docs, arg),
            other => Err(error::expected(tokens, "named or positional argument", other).into()),
        }
    }
}
